...
Future<void> removeMovieFromWatchedList(String userId, ListMovies movie) async {
  try {
    // Reference to the user's document in the users collection
    DocumentReference<Map<String, dynamic>> userDocRef =
        FirebaseFirestore.instance.collection('users').doc(userId);
    ...
    // Reference to the favorites subcollection within the user's document
    CollectionReference<Map<String, dynamic>> watched =
        userDocRef.collection('watched');

    // Query for the document corresponding to the movie
    QuerySnapshot<Map<String, dynamic>> snapshot = await watched
        .where('title', isEqualTo: movie.title)
        .limit(1)
        .get();
